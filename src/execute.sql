...
FROM
    ...
    INNER JOIN economies AS e -- Match on code field using table aliases
    ON c.code = e.code;
...
FROM
    countries AS c -- Join to languages (aliased)
    INNER JOIN languages AS l -- Use code as the joining field with the USING keyword
    USING(code);
...
FROM
    countries AS c
    INNER JOIN languages AS l USING(code) -- Order the results by language
ORDER BY
    language;
...
SELECT
    name,
    year,
    fertility_rate -- Inner join countries and populations, aliased, on code
FROM
    countries AS c
...
SELECT
    ...
    city_proper_pop
FROM
    cities AS c1 -- Perform an inner join with cities as c1 and countries as c2 on country code
...
FROM
    ...
    USING(code) -- Filter for the year 2010
WHERE
    year = 2010;
...
ORDER BY
    avg_gdp DESC -- Return only first 10 records
LIMIT
    10;
...
FROM
    ...
    FULL
    JOIN currencies USING (code) -- Where region is North America or null
WHERE
...
FROM
    ...
    FULL
    JOIN currencies AS c2 USING(code)
WHERE
    region LIKE 'M%esia';
...
FROM
    countries AS c -- Join to populations (alias as p) using an appropriate join
    LEFT JOIN populations AS p ON c.code = p.country_code -- Filter for only results in the year 2010
WHERE
    year = 2010 -- Order by life_exp
ORDER BY
    life_exp -- Limit to five records
LIMIT
    5;
...
SELECT
    p1.country_code,
    p1.size AS size2010,
    p2.size AS size2015 -- Join populations as p1 to itself, alias as p2, on country code
FROM
    populations AS p1
    INNER JOIN populations AS p2 USING(country_code);
...
SELECT
    DISTINCT name
FROM
    languages -- Order by the name of the language
ORDER BY
    name;
...
SELECT
    AVG(life_expectancy)
FROM
    populations -- Filter for the year 2015
WHERE
    year = 2015;
...
SELECT
    *
FROM
    populations -- Filter for only those populations where life expectancy is 1.15 times higher than average
WHERE
...
SELECT
    ...
    urbanarea_pop
FROM
    cities -- Filter using a subquery on the countries table
WHERE
    name IN (
        SELECT
            capital
...
GROUP BY
    country -- Order by count of cities as cities_num
ORDER BY
    cities_num DESC,
    country
LIMIT
...
FROM
    cities -- Use subquery to filter city name
WHERE
    name IN (
        SELECT
            capital
    ...
    AND metroarea_pop IS NOT NULL -- Sort and limit the result
ORDER BY
    city_perc DESC
LIMIT
    10;
